...
- [ ] **Step 1: Failing tests** — CLI: plan argv + context stdin → summary stdout, exit 0; garbage stdin → exit 0 + `empty`-plan-style summary with failure note; registry: `time_budget_seconds: -5` → `time-budget` violation; valid budget accepted; `CONTRACT_VERSION == "1.1"`
- [ ] **Step 2: Verify fail**
- [ ] **Step 3: Implement** CLI `main()`; `time-budget` rule; bump `CONTRACT_VERSION`; v1.1 addendum in contract doc; registry stamp
- [ ] **Step 4: Verify pass + ruff**
- [ ] **Step 5: Commit** — `feat(006): executor CLI; skill contract v1.1 optional time budget`
...
- [ ] **Step 1: Failing tests** — select_plan extraction incl. unroutable-context warning; CLI end-to-end with registered tmp skill → v1.1 shape, outcome mapping, embedded execution summary; invoker tests removed (superseded by executor tests)
- [ ] **Step 2: Verify fail**
- [ ] **Step 3: Implement** — rewire `main()`, keep `route()` seam for selection tests
- [ ] **Step 4: Verify pass (all test files) + ruff**
- [ ] **Step 5: Commit** — `feat(006): router delegates execution to skill executor`

## Task 4: E2E + docs sync + finish

- [ ] **Step 1: Quickstart E2E** — register fails/slow/works skills, route on pre-commit, real commit → partial-failure summary logged, `works` ran (SC-002); clean up
- [ ] **Step 2: Sync CLAUDE.md**; 004 contract addendum pointer (DispatchResult v1.1); check off tasks.md
